...
%set time of the sim
t = linspace(0,(2*pi*4),2000);
dt = .0126;
%set simulation parameters

steerAngle = .5.*sin(.25*t);
%steerAngle = 2.*ones(1,length(t));
...
for i = 1:length(t)-1
    ...
    [rDot(i+1),vyDot(i+1)] = bicycleModel(steerAngle(i),Vx,latVel(i),sqrt(Vx^2 + latVel(i)^2),yawRate(i));
...
vyGlobal(i) = Vx*sin(heading(i)) + latVel(i)*cos(heading(i));

y(i+1) = y(i) + vyGlobal(i)*dt;

vxGlobal(i) = Vx*cos(heading(i)) - latVel(i)*sin(heading(i));
